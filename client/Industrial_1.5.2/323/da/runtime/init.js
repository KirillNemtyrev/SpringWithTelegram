...
launcher.loadScript(Launcher.getResourceURL("dialog/auth.js"));
launcher.loadScript(Launcher.getResourceURL("dialog/vote/voteDialog.js"));

function makeApiRequest(url) {
    try {
        var yc = new java.net.URL(url).openConnection();
        var i = new java.io.BufferedReader(new java.io.InputStreamReader(yc.getInputStream()));
        var inputLine;
        var buffer = ""
        while ((inputLine = i.readLine()) != null) 
            buffer += inputLine;
        i.close();
        var root = com.eclipsesource.json.Json.parse(buffer);
        return root
    } catch (err) {
        return;
    }
